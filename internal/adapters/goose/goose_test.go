...
func TestGooseRenderProducesCustomProviderFile(t *testing.T) {
	...
	// Goose's schema has no api_key field; auth is via api_key_env (env-var NAME)
	// + requires_auth. The literal token is never written (no-secrets-in-git).
	assert.NotContains(t, cp, "api_key", "literal token must not be written; Goose drops unknown api_key")
	assert.Equal(t, "CUSTOM_HS_GW_API_KEY", cp["api_key_env"])
	assert.Equal(t, true, cp["requires_auth"])
	assert.Equal(t, "https://gw/chat/completions", cp["base_url"])
