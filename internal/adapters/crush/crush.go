...
func (a *Adapter) Render(b *canonical.Bundle) (*adapter.FileSet, error) {
	...
	// Provider/model writes are gated on a non-empty gateway URL so an
	// incomplete profile (reachable via --allow-incomplete) never emits a
	// dangling provider reference or an empty default_model that clobbers the
	// user's value. providers/models are unioned so user-defined entries survive.
	if b.Profile.Gateway.URL != "" {
		if providers := common.ProvidersAsCrushMap(&b.Profile); len(providers) > 0 {
			overlay["providers"] = common.UnionNestedMap(existing, "providers", providers)
		}
		if roleModels := common.CrushRoleModels(&b.Profile); len(roleModels) > 0 {
			overlay["models"] = common.UnionNestedMap(existing, "models", roleModels)
		}
